...
# API配置
WEATHER_API_KEY = CONFIG["api"]["weather_api_key"]  # 替换为你的天气API密钥
CITY_ID = CONFIG["api"]["city_id"]
NEWS_API_URL = "https://uapis.cn/api/v1/daily/news-image"

def initialize_models():
    """一次性加载所有AI模型"""
    print(f"[{time.strftime('%H:%M:%S')}] 正在加载AI模型...")
    ...
    return vqa, caption_interface
VQA_MODEL, CAPTION_MODEL = initialize_models()
...
class MotionDetector:
    ...
    def __init__(self, motion_threshold=1500, min_contour_area=500, 
        ...
        self.sleep_motion_threshold = 2000  # 休眠模式下的运动阈值（更敏感）
        ...
        self.initialization_threshold = 10  # 需要10帧稳定初始化

        self.last_analysis_time = 0
        self.analysis_interval = 5.0  # 分析间隔(秒)
        self.analysis_queue = analysis_queue  # 从外部传入
    ...
    def process_frame(self, frame):
        ...
        result = {
            ...
            'thresh': thresh,
            'should_analyze': False,  # 替代should_run_script
        ...
        if self.is_sleeping:
            ...
            if has_motion and motion_area >= self.emergency_threshold:
                if (current_time - self.last_analysis_time) >= self.analysis_interval:
                    ...
                    print(f"[{time.strftime('%H:%M:%S')}] 触发紧急分析")
            
                    return result
            
            # 检测到运动，唤醒系统
            if has_motion and self._should_analyze():
                result['should_analyze'] = True
                self.last_analysis_time = current_time
                print(f"[{time.strftime('%H:%M:%S')}] 触发常规分析")
...
class ModernSmartCalendar:
    def __init__(self, root):
        ...
        self.root.geometry(CONFIG["window"]["geometry"])  # 调整窗口大小
        ...
        self.camera_paused = False  # 新增：摄像头暂停标志
        ...
        try:
            ...
            if if_emergency:
                ...
                if if_suspicious:
                    ...
                    <p><small>此邮件由自动监控系统发送于 {datetime.datetime.now().strftime("%Y-%m-%d %H:%M:%S")}</small></p>
                    """
...
if __name__ == "__main__":
    ...
    def pause_camera(self):
        ...
        if self.cap and self.cap.isOpened():
            self.cap.release()
            self.cap = None
        print(f"[{time.strftime('%H:%M:%S')}] 摄像头已暂停")
    ...
    def resume_camera(self):
        ...
        try:
            self.cap = cv2.VideoCapture(0)
            if not self.cap.isOpened():
                print(f"[{time.strftime('%H:%M:%S')}] 错误：无法重新打开摄像头")
                return False
            ...
            # 新增：恢复时也进行预热
            print(f"[{time.strftime('%H:%M:%S')}] 摄像头重新预热中...")
            for _ in range(5):
                ret, _ = self.cap.read()
                if not ret:
                    time.sleep(0.1)
            ...
            self.camera_paused = False
            print(f"[{time.strftime('%H:%M:%S')}] 摄像头已恢复")
            return True
        except Exception as e:
            print(f"[{time.strftime('%H:%M:%S')}] 摄像头恢复失败: {e}")
            return False
    ...
    def create_weather_card(self):
        ...
        weather_container = tk.Frame(self.content_container, bg="#0f0f1e", width=350)
        weather_container.pack(side=tk.LEFT, fill=tk.BOTH, padx=(0, 15))
        weather_container.pack_propagate(False)  # 固定宽度
    ...
    def create_camera_section(self):
        ...
        camera_container = tk.Frame(self.content_container, bg="#0f0f1e", width=450)
        camera_container.pack(side=tk.LEFT, fill=tk.BOTH, padx=15)
        camera_container.pack_propagate(False)  # 固定宽度
        ...
        try:
            ...
            if response.status_code == 200:
                ...
                if canvas_width <= 1:  # 如果canvas还没有渲染
                    canvas_width = 700  # 使用默认宽度
                
                display_width = canvas_width - 40  # 留一些边距
                img_width, img_height = image.size
    ...
    def camera_loop(self):
        ...
        while self.camera_active:
            ...
            # 处理紧急事件
            if result['should_run_emergency'] and not self.detector.emergency_running:
                self.camera_queue.put("EMERGENCY")
                # 暂停摄像头
                self.pause_camera()
                # 执行紧急脚本
                self.detector.run_emergency_script()
                # 恢复摄像头
                self.resume_camera()
                continue
            
            # 处理脚本执行
            if result['should_run_script'] and not self.detector.script_running:
                self.camera_queue.put("SCRIPT_RUNNING")
                # 暂停摄像头
                self.pause_camera()
                # 执行脚本
                self.detector.run_external_script()
                # 恢复摄像头
                self.resume_camera()
                continue
            ...
            while not self.camera_queue.empty():
                ...
                if isinstance(data, str):
                    if data == "SCRIPT_RUNNING":
                        self.camera_label.config(text="脚本执行中...", image="")
                        self.camera_status_label.config(text="状态: 脚本执行中")
                    elif data == "EMERGENCY":
                        self.camera_label.config(text="紧急事件处理中...", image="")
                        self.camera_status_label.config(text="状态: 紧急事件处理中")
                else:
                    # 是处理后的帧数据
                    frame = data['frame']
                    ...
                    # 调整大小以适应显示区域
                    image = image.resize((410, 308), Image.Resampling.LANCZOS)  # 适应450宽度容器
